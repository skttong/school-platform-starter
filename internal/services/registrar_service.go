...
func (s *RegistrarService) Enroll(ctx context.Context, studentID, classroomID int64, year, term int) (*models.Enrollment, error) {
	if studentID == 0 || classroomID == 0 || year == 0 || term == 0 { return nil, errors.New("missing required fields") }
	var id int64
	if err := s.db.QueryRow(ctx, `INSERT INTO enrollments (student_id, classroom_id, year, term) VALUES ($1,$2,$3,$4) RETURNING id`, studentID, classroomID, year, term).Scan(&id); err != nil { return nil, err }
	return s.Get(ctx, id)
}

func (s *RegistrarService) UpdateStatus(ctx context.Context, id int64, status string) error {
	cmd, err := s.db.Exec(ctx, `UPDATE enrollments SET status=$1 WHERE id=$2`, status, id); if err != nil { return err }
...
func (s *RegistrarService) ListByClassroom(ctx context.Context, classroomID int64, year, term int) ([]models.Enrollment, error) {
	rows, err := s.db.Query(ctx, `SELECT id, student_id, classroom_id, year, term, status, enrolled_at FROM enrollments WHERE classroom_id=$1 AND year=$2 AND term=$3 ORDER BY id DESC`, classroomID, year, term)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []models.Enrollment
	for rows.Next() { var e models.Enrollment; if err := rows.Scan(&e.ID,&e.StudentID,&e.ClassroomID,&e.Year,&e.Term,&e.Status,&e.EnrolledAt); err != nil { return nil, err }; out = append(out, e) }
	return out, nil
}

func (s *RegistrarService) ListByStudent(ctx context.Context, studentID int64) ([]models.Enrollment, error) {
	rows, err := s.db.Query(ctx, `SELECT id, student_id, classroom_id, year, term, status, enrolled_at FROM enrollments WHERE student_id=$1 ORDER BY year DESC, term DESC`, studentID)
	if err != nil { return nil, err }
	defer rows.Close()
	var out []models.Enrollment
	for rows.Next() { var e models.Enrollment; if err := rows.Scan(&e.ID,&e.StudentID,&e.ClassroomID,&e.Year,&e.Term,&e.Status,&e.EnrolledAt); err != nil { return nil, err }; out = append(out, e) }
