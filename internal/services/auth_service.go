...
func (s *AuthService) Hash(pw string) string {
	h := sha256.Sum256([]byte(pw)) // demo: ใช้ bcrypt/argon2 ในโปรดักชัน
...
func (s *AuthService) ValidateUser(ctx context.Context, email, password string) (userID int64, perms map[string]bool, ok bool) {
	var id int64
	var hash string
	if err := s.db.QueryRow(ctx, `SELECT id, password FROM users WHERE email=$1 AND is_active=true`, email).Scan(&id, &hash); err != nil {
		return 0, nil, false
	}
	if hash != s.Hash(password) {
		return 0, nil, false
	}
