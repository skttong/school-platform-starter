...
func (s *ClassroomService) List(ctx context.Context, q string, year, term, page, size int) ([]models.Classroom, int, error) {
	...
	args := []any{}
	where := []string{}
	if q != "" { args = append(args, "%"+strings.ToLower(q)+"%"); where = append(where, "(LOWER(name) LIKE $1 OR LOWER(code) LIKE $1)") }
	if year > 0 { args = append(args, year); where = append(where, fmt.Sprintf("year=$%d", len(args))) }
	if term > 0 { args = append(args, term); where = append(where, fmt.Sprintf("term=$%d", len(args))) }
	w := ""; if len(where) > 0 { w = "WHERE "+strings.Join(where, " AND ") }
	sqlList := fmt.Sprintf(`SELECT id, code, name, grade, year, term, homeroom_teacher, capacity, created_at, updated_at FROM classrooms %s ORDER BY year DESC, term DESC, name ASC LIMIT %d OFFSET %d`, w, size, offset)
	rows, err := s.db.Query(ctx, sqlList, args...); if err != nil { return nil, 0, err }
	defer rows.Close()
	var items []models.Classroom
	for rows.Next() { var it models.Classroom; if err := rows.Scan(&it.ID,&it.Code,&it.Name,&it.Grade,&it.Year,&it.Term,&it.HomeroomTeacher,&it.Capacity,&it.CreatedAt,&it.UpdatedAt); err != nil { return nil,0,err }; items = append(items, it) }
	var total int; if err := s.db.QueryRow(ctx, "SELECT COUNT(*) FROM classrooms "+w, args...).Scan(&total); err != nil { return nil,0,err }
...
func (s *ClassroomService) Create(ctx context.Context, it *models.Classroom) error {
	if it.Code == "" || it.Name == "" || it.Year == 0 || it.Term == 0 { return errors.New("missing required fields") }
	return s.db.QueryRow(ctx, `INSERT INTO classrooms (code,name,grade,year,term,homeroom_teacher,capacity) VALUES ($1,$2,$3,$4,$5,$6,COALESCE($7,50)) RETURNING id`,
...
func (s *ClassroomService) Delete(ctx context.Context, id int64) error {
	cmd, err := s.db.Exec(ctx, `DELETE FROM classrooms WHERE id=$1`, id); if err != nil { return err }
	if cmd.RowsAffected() == 0 { return pgx.ErrNoRows }
	return nil
}
